...
import com.wangcl.constant.RoleConstant;
import com.wangcl.mapper.UserMapper;
import com.wangcl.model.User;
...
public class UserServiceImpl implements UserService {
    ...
    public String insert(User user) {
        ...
        if("male".equals(user.getGender())){
            user.setAvatarImgUrl("img/user/noLogin_male.jpg");
        } else {
            user.setAvatarImgUrl("img/user/noLogin_female.jpg");
        }
...
//        密码修改成功后注销当前用户
    ...
    public int findIdByUsername(String username) {
        int userid;
        userid = (int)userMapper.findIdByUsername(username);
        if ("".equals(userid+"")){
            return 1;
        }else{
            return userid;
